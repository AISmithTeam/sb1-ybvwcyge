...
import { Phone, Copy, ToggleLeft, ToggleRight } from 'lucide-react';
...
const PhoneNumberCard = ({ phoneNumber, onDelete, onEdit, onToggleStatus }: PhoneNumberCardProps) => {
  ...
  return (
    <Card variant="glass" className="group hover:shadow-lg transition-shadow duration-200">
      ...
      <div className="px-6 py-3 bg-slate-50/50 dark:bg-dark-800/50 border-t border-slate-200/50 dark:border-dark-700/50">
        <button
          onClick={() => onToggleStatus(phoneNumber.id)}
          className="w-full flex items-center justify-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors"
        >
